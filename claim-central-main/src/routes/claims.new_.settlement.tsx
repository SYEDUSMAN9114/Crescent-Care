...
export default function ClaimSettlement() {
  ...
  return (
    <AppShell
      ...
      subtitle="Claim settlement entry"
      actions={
      ...
      <div className="space-y-5">
        <Section title="Claim Settlement">
          <div className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field
              label="Client Name"
              value="Sindh Microfinance Bank Ltd"
              wide
            />
            <Field label="Account #" />
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-3">
            <div className="rounded-xl border border-border bg-surface-2 p-3">
              Total Loss: <span className="font-semibold">11,970</span>
            </div>
          </div>
        </Section>

        <Section
          title="Payments Breakup"
          desc="Add payment lines for this settlement"
        >
                  ...
                  <tr key={r.id} className="border-b border-border/70">
                    ...
                    <td className="px-3 py-3">
                      <select
                        value={r.paymentType}
                        onChange={(e) =>
                          updateRow(r.id, { paymentType: e.target.value })
                        }
                    ...
                    <td className="px-3 py-3">
                      <select
                        value={r.payeeType}
                        onChange={(e) =>
                          updateRow(r.id, { payeeType: e.target.value })
                        }
                        className="field-underline"
                      >
                    ...
                    <td className="px-3 py-3">
                      <input
                        value={r.payeeName}
                        onChange={(e) =>
                          updateRow(r.id, { payeeName: e.target.value })
                        }
                        className="field-underline"
                      />
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <input
                        value={r.nature}
                        onChange={(e) =>
                          updateRow(r.id, { nature: e.target.value })
                        }
                        className="field-underline"
                      />
                    </td>
          ...
          <div className="mt-3 flex gap-2">
            <button
              onClick={addRow}
              className="inline-flex items-center gap-2 rounded-xl border border-dashed border-border px-3 py-2 text-xs font-medium text-muted-foreground hover:border-primary/50 hover:text-primary"
            >
              Add row
            </button>
          </div>
