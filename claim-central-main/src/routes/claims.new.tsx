...
export const Route = createFileRoute("/claims/new")({
  validateSearch: (search: Record<string, unknown>) => ({
    ...
    entry: typeof search.entry === "string" ? Number(search.entry) : undefined,
...
function NewClaim() {
  ...
  const [additionalStay, setAdditionalStay] = useState("");
  const [showSecondaryDiagnosis, setShowSecondaryDiagnosis] = useState(false);
  const [showAdditionalApproved, setShowAdditionalApproved] = useState(false);
  const [showAdditionalStay, setShowAdditionalStay] = useState(false);
  ...
  const revisionSource = useMemo(
    () =>
      isRevision
        ? claims.find(
            (c) =>
              c.intimationNo === revision &&
              c.entryNo === (revisionSourceEntry ?? 1),
          )
        : undefined,
    [isRevision, revision, revisionSourceEntry],
  );
  const [claimNo] = useState(revision ?? "00001");
  const [entryNo] = useState(
  ...
  useEffect(() => {
    ...
    setPatientId(matchedPolicy.dependent[0]?.id ?? "");
    setCauseOfLoss(
      revisionSource.causeOfLoss.includes("Maternity") ? "Maternity" : "IPD",
    );
    setSelectedBenefit(
      revisionSource.causeOfLoss.includes("C-Section")
        ? "C-Section"
        : revisionSource.causeOfLoss,
    );
  ...
  return (
    <AppShell
      title={
        isRevision
          ? `${revisionMode === "limit" ? "Limit Enhancement" : revisionMode === "bill" ? "Bill Enhancement" : "Reimbursement Revision"} — Claim Revision`
          : "New Claim Intimation"
      }
      subtitle={`${isRevision ? "Previous claim" : "Claim no."} ${claimNo} · Entry ${entryNo}${policy ? ` · ${policy.clientName}` : ""}`}
      actions={
